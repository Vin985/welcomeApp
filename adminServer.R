...
observeEvent(input$submit, {
  ...
  if (input$id != "0") { # it is a selected account to be updated  
    UpdateData(formData())
...
output$responses <- DT::renderDataTable({
  ...
  req(isAdmin(USER$username, USER$password))
...
CreateDefaultRecord <- function() {
  mydefault <- CastData(list(id = "0", username = "", password = "", email = ""))
  return (mydefault)
}
...
DeleteData <- function(data) {
  ...
  if(user["admin"] == 1){
    output$crudErrors <- renderUI({
      span("Opération non autorisée. Un compte administrateur ne peut être supprimé.", class ="text-danger")  
    })
  }
  else{
    output$crudErrors <- renderUI({})
